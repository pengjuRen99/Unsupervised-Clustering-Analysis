...
class VaDE(nn.Module):
    ...
    def pre_train(self, dataloader, pre_epoch=10):
        if not os.path.exists('pytorch/pretrain_model.pk'):
            Loss = nn.MSELoss()
            ...
            self.pi_.data = torch.from_numpy(gmm.weights_).to(self.args.device).float()
            self.mu_c.data = torch.from_numpy(gmm.means_).to(self.args.device).float()
            self.log_sigma2_c.data = torch.log(torch.from_numpy(gmm.covariances_).to(self.args.device).float())
            
            torch.save(self.state_dict(), 'pytorch/pretrain_model.pk')

        else:
            self.load_state_dict(torch.load('pytorch/pretrain_model.pk'))
